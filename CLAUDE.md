...
- **Request logging** (`src/common/logging/logging.middleware.ts`): `loggingMiddleware` logs every request as `` `${method} ${url} ${statusCode} ${durationMs}ms` `` via `response.on('finish', ...)`. It's registered as plain Express middleware in `main.ts`, deliberately **not** an `APP_INTERCEPTOR` — interceptors only run for requests that already passed Guards, so a `NestInterceptor` here would silently miss 401s (`AccessTokenGuard`) and 429s (`ThrottlerGuard`); middleware runs first in the pipeline and catches every request regardless of how it's rejected.
